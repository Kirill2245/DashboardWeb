...
const Table = ({showProfile, onDataSend}) => {
    const data = [
        {
            name:"John Deo",
        ...
        <div>
            <table className= {styles.table}>
                ...
                <tbody>
                    {data.map((item, index) => {
                        return(
                            <tr onClick={() => {showProfile();onDataSend({email:item.email, phone:item.phone, name:item.name, img: item.img })}} key={index}>
                                <td><div className={styles.box}><img src={item.img}/><p>{item.name}</p></div></td>
                                <td>{item.email}</td>
                                <td>{item.phone}</td>
